...
import { Brain, Eye, Cloud, Database, MessageSquare, Settings, Layers } from "lucide-react";
...
const skillCategories: SkillCategory[] = [
    { title: "Machine Learning", icon: Brain, skills: ["Scikit-learn · XGBoost · LightGBM", "Feature Engineering & Selection", "Cross-validation & Model Evaluation", "Gradient Boosting & Ensemble Methods", "Classification, Regression & Clustering"] },
    ...
    return (
        <section id="skills" className="py-24 px-6 bg-[#111318] relative overflow-hidden">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="w-[600px] h-[400px] rounded-full bg-[#F59E0B]/[0.03] blur-[130px]" />
            </div>
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                <div className="text-center mb-14">
                    ...
                    <motion.div
                        ...
                        className="h-[2px] bg-[#F59E0B]/50 mx-auto rounded-full"
                    />
                    <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="mt-6 text-slate-400 max-w-2xl mx-auto font-medium">
                        End-to-end stack for building intelligent systems — from raw data and model training to production-grade AI deployment.
                    </motion.p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {skillCategories.map((category, index) => (
                        <motion.div
                            ...
                            onMouseEnter={e => {
                                (e.currentTarget as HTMLElement).style.boxShadow = `0 16px 48px rgba(0,0,0,0.5), 0 0 40px rgba(245,158,11,0.06)`;
                        ...
                        >
                            ...
                            <div className="absolute top-0 left-0 w-full h-[2px] bg-[#F59E0B]/60 opacity-0 group-hover:opacity-100 transition-opacity" />

                            <div className="flex items-center gap-4 mb-6">
                                <div className="p-3 rounded-xl bg-[#F59E0B]/10 transition-colors group-hover:bg-[#F59E0B]/15">
                                    <category.icon size={26} className="text-[#F59E0B]" />
                                </div>
                                <h3 className="text-[17px] font-bold text-white leading-tight">{category.title}</h3>
                            </div>

                            <ul className="space-y-2.5 flex-grow">
                                {category.skills.map((skill, sIndex) => (
                                    <li key={sIndex} className="flex items-start gap-2.5">
                                        <span className="mt-[7px] shrink-0 w-[4px] h-[4px] rounded-full bg-[#F59E0B]/40" />
                                        <span className="text-slate-400 text-[13.5px] leading-relaxed group-hover:text-slate-300 transition-colors">{skill}</span>
                                    </li>
                                ))}
                            </ul>
                        </motion.div>
