...
export default function LandingPage() {
    ...
    return (
        <main className="min-h-screen bg-[#111318] selection:bg-[#F59E0B]/15 relative overflow-hidden">
            ...
            <AnimatePresence>
                {isLoading && (
                    <motion.div key="loader-container" className="fixed inset-0 z-[100] flex flex-col items-center justify-center px-6" exit={{ opacity: 0 }} transition={{ duration: 0.8, ease: "easeInOut" }}>
                        {/* Profile Photo */}
                        <motion.div initial={{ opacity: 0, scale: 0.85, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.34, 1.56, 0.64, 1] }} className="relative mb-[30px] z-20">
                        ...
                        <motion.div animate={{ opacity: phase === "transitioning" ? 0 : 1, y: phase === "transitioning" ? 40 : 0 }} transition={{ duration: 0.4, ease: "easeIn" }} className="flex flex-col items-center text-center w-full">
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4, duration: 0.3 }} className="mb-[40px]">
                            ...
                            <div className="w-full max-w-[300px] md:max-w-[450px] mb-[35px]">
                                <div className="flex justify-between items-end mb-[12px] font-mono text-[13px]">
                                    <div className={status === "DEPLOYMENT_COMPLETE" ? "text-[#F59E0B] font-bold" : "text-[#F59E0B]/50"}>
                                        {status}
                            ...
                            {/* Stats Card */}
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 0.6 }} className="bg-[#1c2030]/90 backdrop-blur-[12px] border border-white/10 rounded-[16px] p-8 shadow-[0_8px_32px_rgba(0,0,0,0.4)] mb-[25px]">
                            ...
                            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8, duration: 0.5 }} className="italic text-[15px] text-slate-500 font-medium">
                ...
                {(!isLoading || phase === "transitioning") && (
                    <motion.div key="portfolio-content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.2, ease: "easeOut" }} className="relative z-10">
                        <ScrollProgress />
                        <Navbar />
